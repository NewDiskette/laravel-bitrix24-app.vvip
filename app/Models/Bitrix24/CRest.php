...
class CRest
{
	...
	{
		$return = [];
		if(file_exists('../bitrix24/settings.json'))
		{
			$return = static::expandData(file_get_contents('../bitrix24/settings.json'));
	...
	{
		return  (boolean)file_put_contents('../bitrix24/settings.json', static::wrapData($arSettings));
	}
	...
	public static function checkServer($print = true)
	{
		...
		//creat setting file
		file_put_contents('../bitrix24/settings_check.json', static::wrapData(['test'=>'data']));
		if(!file_exists('../bitrix24/settings_check.json'))
		{
			$return['setting_creat_error'] = 'Check permission! Recommended: folders: 775, files: 664';
		}
		unlink('../bitrix24/settings_check.json');
